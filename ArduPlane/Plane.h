...
#define CTRL_ID 0x01            //управляющие команды на сервы

#define PWR_ 8087821 //P=80 W=87 R=82 + 1
#define	SRT 8382842 // + 2...
#define SOP 8379803
#define STA 8384654
#define SCP 8367805
#define SE1 8369496
...
private:
	...
	AP_Mission::Mission_Command _cmd_for_elev;
	AP_HAL::AnalogSource* chan_ADC_my;
	float 	v_ADC_my 					= 0;
	int8_t 	pin_ADC_my 					= 4;
	bool 	start_work_chocko 			= false;
	bool 	start_first_prestep_chocko 	= false;
	
	float	pitch_for_aspeed			= 10;
	bool 	pitch_less_spedd			= false;
	bool 	drop_pitch_flag 			= true;
	
	float 	Dist_to_target_point 		= 0;
	float 	Course_to_target_point 		= 0;
	
	int 	_servo_on_pwm_chute 		= 1100;
	int 	chute_count_test 			= 0;
	int 	counter_chute 				= 0;
	bool 	parachute_flag				= false;
	
	float 	dive_point_alt 				= 1500;
	float 	dive_point_dist 			= 2000;
	bool 	nose_earth 					= false;
	...
	// float 	target_point_lng 			= 0;
	// float 	target_point_alt 			= 0;
	
	float 	target_point_lat 			= 30.24547917;		//30.238622;
	float 	target_point_lng 			= 30.36686483;		//30.363991;
	float 	target_point_alt 			= 12;
	float 	dist_nose_down 				= 30;
	
	const struct Location &loc5 = gps.location(0);
	
	float x_testt_angle = 0;
    ...
    uint8_t ID 			= CTRL_ID;
    int16_t ctrl[10] 	= {0};
    //ctrl_ailerons 	[-100..100]
    //ctrl_elevator 	[-100..100]
    //ctrl_rudder 		[-100..100]
    //ctrl_throttle 	[-100..100]
    //ctrl_flaps 		[-100..100]
    //ctrl_airbrk 		[-100..100]
    //ctrl_steering 	[-100..100]
    //ctrl_user1 		[-100..100]
    //ctrl_user2 		[-100..100]
    //ctrl_user3 		[-100..100]
	} __attribute__((packed)) t_ctrl_data;  //control data from pixhawk
	
	t_ctrl_data data_for_stend;
	
	bool 		camera_controll = false;
	
	// Command from camera trecking
	int32_t 	diffMouseX;
	int32_t 	diffMouseY;
	char   		buf_cam[10];
	// char 		buf_cam322[14];
    ...
    ///For Sina
    uint16_t values_test_sina[14]   = {};
    float roll_value_pid            = 0;
    float pitch_value_pid           = 0;
	
	bool 		drop_elevator 	    = false;
    
    //sina throttle
    bool        start_sina          = false;
    bool        start_eng_sina      = false;
    int         start_cl_sina       = 0;
    int         delay_sina          = 0;
    float       sina_alt            = 100;
    float       dive_ang_sina       = 0;
    
    float 	target_point_lat_sina 	= 29.8004767;       //29.798986;		//30.238622;
	float 	target_point_lng_sina 	= 31.0847265;       //31.084444;		//30.363991;
	float 	target_point_alt_sina 	= 127;
    
    float   leftail_ctrlval         = 0;
    float   rightail_ctrlval        = 0;
	
	float	throttle_value_sina		= 70;
	float	comthrval_sina			= 0;
	float	old_comthrval_sina		= 0;
	float 	elev_val_const			= 1100;
	float	ail_val_const			= 1200;
    
	/////////////////////////////////////
	////// Connect to X-plane ///////////
	/////////////////////////////////////
	
	float trueSpd_Xplane 	= 0;//
	float w_y_XPlane 		= 0;
	float w_x_Xplane 		= 0;
	float w_z_Xplane 		= 0;
	float pitch_Xplane 		= 15;//
	float roll_Xplane 		= 15;//
	float hding_true_Xplane = 33;//
	float hding_mag_Xplane 	= 25;//
	float alpha_Xplane 		= 0;
	float beta_Xplane 		= 0;
	float lat_Xplane 		= 29.8004255;//
	float lon_Xplane 		= 31.0845172;//
	float alt_Xplane 		= 88;//
	float vvi_XPlane		= 0;//
	float GroundSpd_Xplane  = 0;
    
	float Lailn1_Xplane 	= 0;
    float Railn1_Xplane 	= 0;
    float elev1_Xplane 		= 0;
	float elev1_2_Xplane	= 0;
    bool  senddata_t2 	    = true;
    int   bRec 			    = 0;
    uint8_t* 	messReceive;
    char simbol;
    ...
    */
    ...
    float set_xpl_var(int xpl_start_var);
	...
	//void send_mes_to_stend(AP_HAL::UARTDriver *uart, const char *name);
	...
	void stabilize_speed(float speed_scaler);
	//
    ////////////////////////
    ////// X-Plane /////////
    ////////////////////////
    uint8_t CRC8Sum(uint8_t* data, size_t length);
    float GetFloatValue_(char fir, char sec, char thr, char fou);
    void ReadDataFromXplane();
    uint8_t* SendMessToXplane(float Lailn1, float Railn1, float elev1, float elev1_2);
	uint8_t* SendMessToXplane2(float elev1, float Ailn1, float rudr1, float throl1_from_AP,
